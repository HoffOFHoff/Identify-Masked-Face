...
import pandas as pd
# The package used in Mask_adding.py
import face_recognition
from PIL import Image, ImageDraw
import numpy as np
import os
import re
# The package used in GUI.py
import dlib
import numpy as np
import cv2
import os
...
# The package used in Samility_detected.py
...
# Mask_adding.mask_img(ori_img_dir, mask_img_dir, facelandmarks)
# GUI.show_GUI()
# worked2.show_the_user()
# Samility_detect.image_show()
...
class Face_Register:
    def __init__(self):
        ...
        self.label_faceinstorage = tk.Label(
            self.frame_right, text=str(self.existing_faces))
        self.input_name = tk.Entry(self.frame_right)
        self.input_name_box = ""
        self.label_face_in_frame = tk.Label(self.frame_right)
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces == 1:
                if not self.out_of_range_flag:
                    ...
                    cv2.imwrite(self.current_face_path + "/img_face_" +
                                str(self.ss) + ".png", self.face_ROI_image)
    ...
    def GUI_info(self):
        ...
        self.label_faceinstorage.grid(
            row=2, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right,
                 text="Faces in current frame: ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_face_in_frame.grid(
        ...
        tk.Label(self.frame_right, text="Name: ").grid(
        ...
        # Show log in GUI
        self.log_total.grid(row=11, column=0, columnspan=20,
        ...
        if ret:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    # If the size of ROI > 600*480, output "out of range"
                    if (d.right() + self.ww) > 600 or (d.bottom() + self.hh > 480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
                    ...
                    self.current_frame = cv2.rectangle(self.current_frame,
                                                       ...
                                                       color_rectangle, 2)
            self.current_frame_faces = len(faces)
...
# main body of the face recognition interface
def interface():
    print("If you want to take photos enter 1")
    print("If you want to come to the Facial recognition Login 2")
    print("If you want to exit enter q")
    test = True

    while test:
        # you can register your username and password here
        number_entered = input("Enter your choice:")
        if number_entered == "1":
            show_GUI()
            # Load the png file and use face_recognition to get the numpy type
            cur_direc = os.getcwd()
            path = os.path.join(cur_direc, 'data/faces_from_camera/')
            ori_img_dir=glob.glob(path+'*.png')[-1]
            print(ori_img_dir)
            ...
            # marked_face.show()
            mask_img_dir="images/surgical.png"
            cur_direc = os.getcwd()
            path = os.path.join(cur_direc, 'data/faces_from_camera/')
            ori_img_dir=glob.glob(path+'*.png')[-1]
            mask_img(ori_img_dir, mask_img_dir, facelandmarks)
            The_user_face = show_the_user()
            The_output_face = 'The user who is currently using face recognition is' + The_user_face
            output_result(The_output_face)
            plt.show
            print("See you!Thant's the end.")
            os._exit(0)


        # you can log in and check your messages
        if number_entered == "2":
            ssim_real, ssim_masked = image_show(nrows=1, ncols=1, cmap='gray')
            if ssim_real - ssim_masked < 0.8:
                print('Here is the same person')
                plt.tight_layout()
                plt.savefig(
                    'Result/comparison_plot.png', bbox_inches='tight')
                plt.show()

        # you can exit the program here
        if number_entered == "q":
            test = False
            print("See you!That's the end.")
            os._exit(0)

def output_result(The_output_face):
    f = open("The face result.txt", 'a')
    f.write(The_output_face)
    f.close()
